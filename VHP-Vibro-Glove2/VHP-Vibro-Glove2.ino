...
void setup() {
  ...
  g_csense = new CSense(g_settings.samplerate / 8,
...
void OnPwmSequenceEnd() {
    static uint32_t active_channel = 10;
	...
	for(uint32_t i = 0; i < g_stream->channels(); i++)  {
	    ...
	    if(g_stream->chan_is_currently_playing(i)) {
		// has the active channel changed?
		if(active_channel != i) {
		    //Serial.print("Activec channel:");
		    Serial.println(i);
		    active_channel = i;
		    ...
		    g_dlog.log(i, adc_value);		    
		}
	    }
	}
    
    } else {
	// If not running, just play silence
